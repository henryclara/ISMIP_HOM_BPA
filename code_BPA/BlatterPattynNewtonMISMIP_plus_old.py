...
for dt in dts:
    for theta_out in theta_outs:
        ...
        zb.interpolate(mismip_bed(x, y))
        thick.interpolate(1000.0)
        zs.interpolate(zb + thick)
        mesh.coordinates.interpolate(as_vector([xref, yref, zb + sigmaref * thick]))

        u_init = as_vector([100.0, 0.0])
        ...
        outfile = VTKFile(f"MISMIP_plus_output_dt{dt:g}_theta{theta_out:g}_A1e-25_nx100_beta1000.pvd")
        ...
        for i in range(num_TS):
            for j, n in enumerate(ns):
                ...
                af = rhoi * thick + rhow * zb
                delta_gl = Constant(20.0)
                grounded = 0.5 * (1.0 + tanh(af / (rhoi * delta_gl)))
                ...
                if theta_out != 0:
                    ...
                    # Third term
                    F += theta * rhoi * g * cos(psi) * dt * (((1 - zeta) * rhoi - rhow)/(rhoi - rhow)) * q * (v1.dx(0) + v2.dx(1)) * dx
                
                    # Fourth term
                    F += 0.5 * theta * g * cos(psi) * dt * (ux * (zs * zs).dx(0) + uy * (zs * zs).dx(1)) * (v1.dx(0) + v2.dx(1)) * (1 / sqrt(1 + zs.dx(0)**2 + zs.dx(1)**2)) * ds_t

                    # Fifth term
                    F += theta * rhoi * g * cos(psi) * dt * (((1 - zeta) * rhoi - rhow)/(rhoi - rhow)) * zs * ((v1.dx(0) + v2.dx(1)) / sqrt(1 + zs.dx(0)**2 + zs.dx(1)**2)) * q * ds_t
                ...
                F += rhoi * g * cos(psi) * zs * (v1.dx(0) + v2.dx(1)) * dx #\
                            #+ rhoi * g * sin(psi) * v1 * dx
            ...
            if abs(t % 50.0) < 1.0e-10 or abs((t % 50.0) - 50.0) < 1.0e-10:
                ...
                uout.sub(0).project(uvec_out.sub(0))
                uout.sub(1).project(uvec_out.sub(1))
                uout.sub(2).assign(0.0)
